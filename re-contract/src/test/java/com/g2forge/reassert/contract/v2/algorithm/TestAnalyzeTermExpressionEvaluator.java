...
public class TestAnalyzeTermExpressionEvaluator {
	...
	public void test() {
		...
		final AnalyzeTermExpressionEvaluator.Analyzed actual = new AnalyzeTermExpressionEvaluator(TermRelation.Included).eval(expression);
		HAssert.assertEquals(new AnalyzeTermExpressionEvaluator.Analyzed(expression, HCollection.asSet(TestUsageTerm.Term, TestLicenseTerm.Permission), HCollection.asSet(TestUsageTerm.Term)), actual);
	}
}
