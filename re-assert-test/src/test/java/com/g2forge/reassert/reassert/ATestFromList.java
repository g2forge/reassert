...
import com.g2forge.reassert.reassert.convert.ExpressionModule;
import com.g2forge.reassert.reassert.convert.contract.ContractModule;
import com.g2forge.reassert.reassert.convert.contract.ContractParser;
	...
	public ListRepository computeRepository() {
		...
		final ListRepository repository = new ListRepository(context) {
			...
			protected ObjectMapper computeMapper() {
				...
				mapper.registerModule(new ContractModule(context::describe, new ContractParser(context.getLicenseParser(), context.getUsageParser())));
				mapper.registerModule(new StandardTermModule());
				mapper.registerModule(new ExpressionModule(context::describe));
