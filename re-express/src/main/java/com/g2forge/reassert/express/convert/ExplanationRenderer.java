	...
	protected static class ExplanationRendering<Name, Value> extends ARenderer.ARendering<IExplained<Value>, IExplanationRenderContext<Name, Value>, IExplicitRenderable<? super IExplanationRenderContext<Name, Value>>> {
		...
		protected void extend(TypeSwitch1.FunctionBuilder<IExplained<Value>, IExplicitRenderable<? super IExplanationRenderContext<Name, Value>>> builder) {
			...
			builder.add(ExplainedOperation.class, e -> c -> {
				...
				final List<Child<Name, Value>> childrenPrintable = toPrintable(context.getMode(), childrenAll.stream());
		...
		protected List<Child<Name, Value>> toPrintable(ExplanationMode mode, Stream<? extends Child<Name, Value>> stream) {
			final List<Child<Name, Value>> retVal = stream.filter(Child::isPrintable).collect(Collectors.toList());
			if (retVal.isEmpty() || (mode.compareTo(ExplanationMode.Trace) >= 0)) return retVal;
			final Relevance max = retVal.stream().map(Child::getRelevance).max(ComparableComparator.create()).get();
			return retVal.stream().filter(child -> child.getRelevance().compareTo(max) >= 0).collect(Collectors.toList());
		}
	}
