...
public class ExplanationRenderer<Name, Value> extends ATextualRenderer<IExplained<Value>, IExplanationRenderContext<Name, Value>> {
	...
	protected static class ExplanationRendering<Name, Value> extends ARenderer.ARendering<IExplained<Value>, IExplanationRenderContext<Name, Value>, IExplicitRenderable<? super IExplanationRenderContext<Name, Value>>> {
		...
		protected void extend(TypeSwitch1.FunctionBuilder<IExplained<Value>, IExplicitRenderable<? super IExplanationRenderContext<Name, Value>>> builder) {
			...
			builder.add(ZeroExplainedOperation.class, e -> c1 -> {
				...
				if (context.getMode().compareTo(ExplanationMode.Describe) <= 0) {
					...
					if ((arguments != null) && !arguments.isEmpty()) {
						final List<PrintableArgument<Value>> printableArguments = arguments.stream().map(a -> new PrintableArgument<>(context.getMode(), a)).filter(PrintableArgument::isPrintable).collect(Collectors.toList());
			...
			builder.add(ExplainedOperation.class, e -> c -> {
				...
				/*context.value(expression.get()).append(" (").append(e.getOperator()).append(")").append(" - because ");
				final List<IExplainedOperation.Argument<Value>> arguments = expression.getArguments();
				if ((c.getMode().compareTo(ExplanationMode.Describe) <= 0) && (arguments.stream().filter(IExplainedOperation.Argument::isRelevant).collect(Collectors.counting()) == 1)) {
					...
					if (arguments.size() == 1) {
						context.append("there's a single argument");
						printArguments = context.getMode().compareTo(ExplanationMode.Trace) >= 0;
						if (!printArguments) context.append(": ").render(HCollection.getOne(arguments).getExplained(), IExplained.class);
					} else {
					...
					if (printArguments && (arguments != null) && !arguments.isEmpty()) {
						...
						if (!printableArguments.isEmpty()) {
							...
							try (final ICloseable indent = c.indent()) {
								boolean first = true;
								for (IExplainedOperation.Argument<Value> argument : printableArguments) {
									if (first) first = false;
									else c.newline();
		...
		/*protected boolean isSkipped(IExplanationRenderContext<Name, Value> context, IExplainedOperation.Argument<Value> argument) {
	...
	protected static class PrintableArgument<Value> implements IExplained<Value> {
		...
		protected String computePrefix() {
			final boolean trace = mode.compareTo(ExplanationMode.Trace) <= 0;
